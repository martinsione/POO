...
class Alumno {
public:
    Alumno(string nombre, int nota1, int nota2, int nota3) : m_nombre(nombre), m_nota1(nota1), m_nota2(nota2), m_nota3(nota3) {}
...
class Materia {
public:
    Materia(vector<Alumno> alumnos, string nombreProfesor, string nombreMateria) : m_alumnos(alumnos), m_nombreProfesor(nombreProfesor), m_nombreMateria(nombreMateria) {}
    vector<int> Condiciones() {
        ...
        for (int i=0; i<m_alumnos.size(); i++) {
            ...
            if (condicion == 'L') aux_condiciones[0] += 1;
            if (condicion == 'R') aux_condiciones[1] += 1;
            if (condicion == 'P') aux_condiciones[2] += 1;
        }
    ...
    tuple<float, char> BuscarCondicionYPromedio(int dni) {
        int index = -1;
        for (int i=0; i<m_alumnos.size(); i++) {
            if (dni == m_alumnos[i].VerDni()) { index = i; break; }
        }
...
int main() {
    ...
    for (int i=0; i<alumnos.size(); i++) {
        cout << "El alumno con dni: " << alumnos[i].VerDni() << " esta en la condicion de: " << alumnos[i].VerCondicion() << endl;
    }
