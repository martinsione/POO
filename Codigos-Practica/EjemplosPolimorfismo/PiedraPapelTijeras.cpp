...
// esta clase define "qué" puede hacer un jugador (qué métodos debe tener)
// pero no implementa todo, porque hay cosas que solo las hijas sabran "cómo"
class Jugador {
public:
	
	// la clase se encarga de guardar y devolver el nombre del jugador
	Jugador(string nombre) : m_nombre(nombre) { }
	string Nombre() const { return m_nombre; }
	
	// la hija debe decir (sí o sí, por eso el =0) cómo elige su jugada
	virtual string Jugar() =0;
	
	// la hija puede usar para algo el resultado de la partida, o puede 
	// simplemente ignorarlo (porque acá hay una implementación por las 
	// dudas, que no hace nada)
	virtual void Resultado(bool gane) { /*me da igual cómo me fue*/ }
	
	// toda clase con métodos virtuales debe tener su destructor también virtual
	virtual ~Jugador() { }
...
// juega cada uno y quien gana)... está acá (antes de las herencias) para enfatizar
// que con saber que son dos "jugadores" (genéricos, no importa cual) ya alcanza
// para definir la lógica general del juego (solo necesita saber van a estar los
// métodos Jugar, Resultado y Nombre... pero ni le interesa cuales son en 
// particular las posibles hijas que de verdad se van a enviar desde el main)
...
// clase para el nivel fácil, casi siempre elige piedra
...
// clase para el nivel difícil... sabe que los humanos tienden a mantener la
