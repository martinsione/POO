...
class Persona {
    public:
        Persona(string nombre, string estadoCivil, int dni, int fechaNac) : m_nombre(nombre), m_estadoCivil(estadoCivil), m_dni(dni), m_fechaNac(fechaNac) {}
        int Edad(int fechaActual) const {
            return (fechaActual - m_fechaNac) / 10000;
        }
        string Nombre() const {
            return m_nombre;
        }
        string EstadoCivil() const {
...
class Alumno : public Persona {
    ...
    float MeritoAcademico() const {
        return m_promedio * m_materiasAprobadas;
    }
    private:
        float m_promedio;
...
class Docente : public Persona {
    public:
        ...
        int Antiguedad(int fechaActual) const {
            return fechaActual - m_fechaIngreso;
        }
    private:
        int m_fechaIngreso;
};
