...
public:
  Cilindro(float peso, float altura, float radio)
      : Tanque(peso), m_altura(altura), m_radio(radio) {}
  float CalcularVolumen() override { return (M_PI * pow(m_radio, 2)); }
...
public:
  Esfera(float peso, float altura, float radio)
      : Tanque(peso), m_altura(altura), m_radio(radio) {}
